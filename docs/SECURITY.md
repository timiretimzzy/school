...
- [x] Invitation tokens are SHA-256 hashed before storage; raw tokens are
      **never** returned to the frontend. Only safe status information
      (`{ success, status, invitation_id, ... }`) is returned by
      the `invite-user` Edge Function.
- [x] The `invite-user` function prevents duplicate pending invitations for
      the same email+tenant (HTTP 409) and tracks `last_invited_at` on
      `staff_profiles` plus `token_delivered` on `tenant_invitations`.
- [x] `csv` file uploads are not used; CSV import is a client-side paste-and-
      parse workflow with no untrusted file storage path.
