...
- **Works:** role-based workspaces with a context switcher, dynamic branding
  ...
  CRUD, student/staff CRUD with email fields, staff invitation issuance with
  role selection and token display (see `docs/INVITATIONS.md`), CSV bulk
  import for students and staff with client-side validation, teacher-assignment-
  scoped attendance and assessments, published-only student/parent result
  visibility, audience/date-scoped announcements, invitation issuance and
  acceptance.
