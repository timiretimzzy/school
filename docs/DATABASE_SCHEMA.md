...
5. `005_email_and_invitation_tracking.sql` — adds `email` and
   `last_invited_at` columns to `staff_profiles` (for the staff-invitation
   workflow), adds `token_delivered` to `tenant_invitations` (tracks
   whether the raw token was surfaced to the inviter), and creates helpful
   indexes on both tables for email-based lookups.
