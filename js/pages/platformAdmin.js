...
function stepReview(stepEl, body) {
  ...
  nav(stepEl, body, {
    ...
    next: async () => {
      ...
      const hasInvitation = Boolean(data?.invitation);
      msg.innerHTML = `<strong>School created!</strong> ${esc(data.tenant.name)} is now onboarded.<br>
        ${hasInvitation
          ? `An invitation for <strong>${esc(admin.email || "")}</strong> has been sent. The invitee should <a href="#/accept-invite" class="link">accept their invitation here</a> after signing in with that email.`
          : admin.email
            ? `The invitation for ${esc(admin.email)} could not be created. Check the email and try again.`
            : `<em>No administrator email was provided. The school has no initial admin — create one via Staff &amp; invite them manually.</em>`
        }`;
      wizard = null;
    },
  });
